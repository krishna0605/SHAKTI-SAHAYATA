import { ChevronRight, ChevronLeft, Menu, PanelLeftClose, Plus, Search, ShieldCheck, UserCircle2 } from 'lucide-react'
...
export default function AdminLayout() {
  ...
  const renderSidebarNavigation = (isCollapsed: boolean, showCollapseControls = true) => (
    <div className="ops-sidebar-inner">
      ...
      <div className="ops-sidebar-scroll">
        {!isCollapsed ? <div className="ops-sidebar-group-label">Operations</div> : null}
            ...
            <Tooltip key={to}>
              <TooltipTrigger asChild>
                <NavLink
                  ...
                  className={({ isActive }) => cn('ops-sidebar-link', isActive && 'active', isCollapsed && 'is-collapsed')}
                >
                  <span className="ops-sidebar-link-icon">
                    <Icon className="h-[1.1rem] w-[1.1rem] shrink-0" />
                  </span>
                  {!isCollapsed ? <span className="ops-sidebar-link-label">{label}</span> : null}
                </NavLink>
              </TooltipTrigger>
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="ops-app-shell" style={{ gridTemplateColumns: collapsed ? '72px minmax(0, 1fr)' : '256px minmax(0, 1fr)' }}>
        <aside className={cn('ops-sidebar hidden lg:block', collapsed && 'collapsed')}>
          {renderSidebarNavigation(collapsed)}
        </aside>

        <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
          <SheetContent side="left" className="border-r border-white/10 bg-[#0b0d12] p-0 text-slate-100 lg:hidden" showCloseButton={false}>
            <SheetHeader className="border-b border-white/10 px-5 py-4 text-left">
              <SheetTitle className="text-slate-100">SHAKTI Operations Console</SheetTitle>
              <SheetDescription className="text-slate-400">Primary admin navigation and live operations entry points.</SheetDescription>
            </SheetHeader>
            <div className="h-full min-h-0 overflow-y-auto">{renderSidebarNavigation(false, false)}</div>
          </SheetContent>
        </Sheet>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                    <div className="flex items-center gap-2 lg:hidden">
                      <Button
                        ...
                        variant="outline"
                        size="icon"
                        className="h-10 w-10 shrink-0 rounded-lg border-white/10 bg-white/[0.03] text-slate-100 hover:bg-white/[0.08]"
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-sm font-semibold text-slate-100">{routeMeta.title}</div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
                    <div className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-center text-xs text-slate-300 sm:text-left">
                      {liveUpdates.isConnected ? 'Live updates' : 'Refresh fallback active'}
                    </div>
                    {pageAction}
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button type="button" variant="ghost" className="w-full rounded-lg border border-white/10 bg-white/[0.03] px-3 text-slate-100 hover:bg-white/[0.06] sm:w-auto">
                          <UserCircle2 className="h-4 w-4" />
                          <span className="hidden sm:inline">{admin?.fullName || 'Admin'}</span>
                          <span className="sm:hidden">Admin menu</span>
                        </Button>
                      </DropdownMenuTrigger>
