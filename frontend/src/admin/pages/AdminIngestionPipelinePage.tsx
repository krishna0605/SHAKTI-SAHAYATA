...
export default function AdminIngestionPipelinePage() {
  ...
  const selectedRow = useMemo(
    () => ingestionQuery.data?.items.find((item) => item.uploadId === selectedUploadId) || null,
    ...
    <div className="space-y-6 min-w-0">
      ...
      <OpsSection title="Ingestion Queue" description="Uploaded datasets, parser output, validation state, normalization linkage, and error pressure across the intake pipeline.">
        <OpsFilterBar title="Queue Filters">
          ...
          <OpsStatusBadge label={`${formatNumber(summary.normalizationQueued)} normalization queued`} tone="warning" />
          <OpsStatusBadge label={`${formatNumber(summary.retriedJobs)} retried`} tone={summary.retriedJobs > 0 ? 'info' : 'neutral'} />
        </OpsFilterBar>
        <OpsDataTable
          columns={[
      ...
      <OpsDrawerInspector
        ...
        onOpenChange={(open) => {
          if (!open) setSelectedUploadId(null)
        }}
        title={selectedRow ? selectedRow.fileName : 'Upload diagnostics'}
        subtitle={selectedRow ? `Upload ${selectedRow.uploadId}` : 'Select an upload row'}
      >
        {selectedRow ? (
