...
export default function AdminAuditTrailPage() {
  ...
  const selectedEvent = useMemo(
    () => activityQuery.data?.items.find((item) => `${item.source}-${item.id}` === selectedEventId) || activityQuery.data?.items[0] || null,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <OpsInspector title={selectedEvent ? titleCase(selectedEvent.action) : 'Event detail'} subtitle={selectedEvent ? `${selectedEvent.source}:${selectedEvent.id}` : 'Select an event row'}>
          {selectedEvent ? (
            <div className="space-y-5">
              ...
              <div className="ops-subpanel">
                ...
                <pre className="overflow-auto rounded-xl border border-border/60 bg-[#0b1322] p-4 text-xs text-slate-100 whitespace-pre-wrap break-words">
