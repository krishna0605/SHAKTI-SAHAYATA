...
export default function AdminNormalizationPage() {
  ...
  const selectedJobSummary = useMemo(
    () => normalizationQuery.data?.jobs.find((job) => job.jobId === selectedJobId) || null,
  ...
  return (
    <div className="space-y-6 min-w-0">
      <OpsSummaryStrip className="xl:grid-cols-5">
      ...
      <OpsSection title="Processing Queue" description="Normalization job queue with confidence, warning counts, and operational timing." className="min-w-0">
        <OpsFilterBar title="Queue Filters">
          ...
          <OpsStatusBadge label={`${formatNumber(summary.unmappedFieldCount)} unmapped`} tone={summary.unmappedFieldCount > 0 ? 'warning' : 'success'} />
          <OpsStatusBadge label={summary.modelVersion} tone="info" />
        </OpsFilterBar>
      ...
      <OpsDrawerInspector
        open={Boolean(selectedJobSummary)}
        onOpenChange={(open) => {
          if (!open) setSelectedJobId(null)
        }}
        title={selectedJobSummary ? `Job ${selectedJobSummary.jobId.slice(0, 8)}` : 'Job detail'}
        subtitle={selectedJobSummary ? titleCase(selectedJobSummary.documentType) : 'Select a processing job'}
      >
        {selectedJobSummary ? (
      ...
      {hasJobs ? (
        <div className={cn('grid min-w-0 gap-6', selectedJob ? 'xl:grid-cols-[1.28fr_0.72fr]' : 'xl:grid-cols-[1.08fr_0.92fr]')}>
          <OpsSection title="Raw vs Standardized Mapping Viewer" description="Human-reviewable mapping logic from raw fields into the controlled investigation schema.">
            {selectedJob ? (
              ...
              <OpsPageState
                compact
                title="Mapping viewer awaiting selection"
                description="Select a normalization job from the queue to inspect raw headers, transforms, and standardized output."
              />
            )}
          </OpsSection>
          ...
          <div className="space-y-6">
            <OpsSection title="Normalization Timeline" description="Stage-by-stage operational progression for the selected job.">
              {selectedJob ? (
                <OpsTimeline items={selectedJob.stageTimeline} />
              ) : (
                <OpsPageState
                  compact
                  title="No timeline available"
                  description="Select a job to reveal uploaded, parsed, validated, normalized, and downstream stages."
                />
              )}
            </OpsSection>
            ...
            <OpsSection title="Conformity & Anomalies" description="Schema health, low-confidence fields, and downstream readiness.">
              {selectedJob ? (
                ...
                <OpsPageState
                  ...
                  description="Conformity and anomaly indicators appear here once a queue row is selected."
                />
              )}
            </OpsSection>
          </div>
        ...
        <OpsSection title="Processing Details" description="Mapping, conformity, and stage information appear here once normalization jobs enter the monitored queue.">
          <OpsPageState
            compact
            title="No normalization jobs in scope"
            description="Adjust the queue filters or wait for a new processing job to reach this workspace. Once a job is available, mapping, timeline, and anomaly detail will open here without expanding the page into empty panels."
          />
        </OpsSection>
      )}
    </div>
